...
function hoursFromDecimalToNow(startHourDecimal) {
  ...
  const nowDecimal = now.getHours() + now.getMinutes() / 60 + now.getSeconds() / 3600;
    ...
    let diff = 0;
    if (nowDecimal <= 12.0) { // before lunch
        diff = nowDecimal - startHourDecimal;
    } else if (nowDecimal > 12.0 && nowDecimal < 13.5) { // at lunch
        diff = 12.0 - startHourDecimal;
    } else { // after lunch
        diff = nowDecimal - startHourDecimal - lunchBreak;
    }
